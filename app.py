...
# --- META TAG PWA + SERVICE WORKER + DB + SYNC JS (per installazione come app su smartphone e offline) ---
st.markdown("""
...
# --- SYNC ENGINE: hidden elements per comunicazione JS ↔ Python ---
SYNC_STATUS_HTML = """
...
"""
st.markdown(SYNC_STATUS_HTML, unsafe_allow_html=True)

# --- GESTIONE PENDING RAPPORTINI (salvati offline in session_state) ---
if "pending_rapportini" not in st.session_state:
    st.session_state.pending_rapportini = []
...
css_code = """
    ...
    .sync-status-online { color: #22c55e; font-size: 12px; }
    .sync-status-offline { color: #ef4444; font-size: 12px; }
    .sync-status-pending { color: #f59e0b; font-size: 12px; }
    </style>
"""
...
# Funzione per badge pending (richiamata da JS)
badge_html = """
<span id="rapportini-pending-badge" class="pending-badge">0</span>
"""
...
# --- AUTO-SYNC: tenta di sincronizzare i rapportini pending all'avvio ---
if st.session_state.pending_rapportini and conn_disponibile and gworksheet is not None:
    with st.spinner("🔄 Sincronizzazione dati in corso..."):
        synced_ok = []
        synced_fail = []
        for idx, pending in enumerate(st.session_state.pending_rapportini):
            try:
                st.session_state.rapportini.append(pending)
                if scrivi_su_google_sheets(gworksheet, st.session_state.rapportini):
                    synced_ok.append(pending)
                else:
                    synced_fail.append(pending)
            except Exception:
                synced_fail.append(pending)
        
        # Rimuovi quelli sincronizzati con successo
        for item in synced_ok:
            st.session_state.pending_rapportini.remove(item)
        
        if synced_ok:
            st.success(f"✅ {len(synced_ok)} rapportino/i sincronizzato/i automaticamente con Google Fogli!")
        if synced_fail:
            st.warning(f"⚠️ {len(synced_fail)} rapportino/i in attesa di sincronizzazione.")

# --- AUTO-SYNC: tenta di sincronizzare anche se arrivano dati da JS (trigger) ---
# Questa sezione legge il trigger hidden div (simulato con session_state)
sync_trigger = st.session_state.get("_sync_trigger_data", None)
if sync_trigger and conn_disponibile and gworksheet is not None:
    try:
        nuovo_da_js = json.loads(sync_trigger) if isinstance(sync_trigger, str) else sync_trigger
        st.session_state.rapportini.append(nuovo_da_js)
        if scrivi_su_google_sheets(gworksheet, st.session_state.rapportini):
            st.success("✅ Rapportino sincronizzato da dispositivo offline!")
        else:
            # Se fallisce, rimuovi dall'elenco principale e rimetti in pending
            st.session_state.rapportini.pop()
            st.session_state.pending_rapportini.append(nuovo_da_js)
            st.warning("⚠️ Rapportino salvato localmente. Verrà sincronizzato quando possibile.")
    except Exception as e:
        pass
    st.session_state._sync_trigger_data = None
...
# --- FUNZIONE PER SALVATAGGIO CON GESTIONE OFFLINE ---
def salva_rapportino_con_offline(nuovo_rapportino):
    """Salva il rapportino: su Google Sheets se online, altrimenti in coda pending + IndexedDB."""
    # Aggiungi sempre a session_state
    st.session_state.rapportini.append(nuovo_rapportino)
    
    if conn_disponibile and gworksheet is not None:
        try:
            if scrivi_su_google_sheets(gworksheet, st.session_state.rapportini):
                return "success", "✅ Rapportino salvato permanentemente su Google Fogli!"
            else:
                # Ripiega in pending
                st.session_state.rapportini.pop()
                st.session_state.pending_rapportini.append(nuovo_rapportino)
                return "pending", "⚠️ Google Sheets non raggiungibile. Rapportino salvato in coda di sincronizzazione."
        except Exception as e:
            st.session_state.rapportini.pop()
            st.session_state.pending_rapportini.append(nuovo_rapportino)
            return "pending", f"⚠️ Google Sheets non raggiungibile ({e}). Rapportino salvato in coda di sincronizzazione."
    else:
        # Modalità offline - salviamo in pending
        # Nota: il rapportino è già stato aggiunto a session_state.rapportini
        # per visualizzarlo subito, ma lo salviamo anche in pending per il sync persistente
        st.session_state.pending_rapportini.append(nuovo_rapportino)
        return "pending", "💾 Rapportino salvato **localmente**. Verrà sincronizzato con Google Fogli automaticamente quando tornerai online."
...
# --- CSS E JS PER AUTO-CHIUSURA SIDEBAR SU MOBILE (affidabile) ---
st.markdown("""
<style>
@media (max-width: 768px) {
    ...
    body.sidebar-closed .sidebar-reopen-btn {
...
(function() {
    ...
    btn.onclick = function() { document.body.classList.remove('sidebar-closed'); };
    document.body.appendChild(btn);

    function closeSidebarIfChecked() {
        var radios = document.querySelectorAll('section[data-testid="stSidebar"] input[type="radio"]');
        var checked = false;
        radios.forEach(function(r) { if (r.checked) checked = true; });
        if (checked) document.body.classList.add('sidebar-closed');
    }

    // Polling ogni 300ms per intercettare click sui radio button (affidabile con Streamlit)
    setInterval(closeSidebarIfChecked, 300);

    // All'avvio, chiudi dopo un attimo
    setTimeout(closeSidebarIfChecked, 1000);
})();
</script>
""", unsafe_allow_html=True)
...
# --- BADGE SYNCRONIZZAZIONE PENDING IN SIDEBAR ---
pending_count = len(st.session_state.pending_rapportini)
if pending_count > 0:
    st.sidebar.markdown(
        f'<div style="background:#fef3c7; border:1px solid #f59e0b; border-radius:8px; padding:8px; margin-bottom:8px;">'
        f'<div style="display:flex; align-items:center; gap:6px;">'
        f'<span>⏳</span>'
        f'<span style="font-size:13px; font-weight:bold; color:#92400e;">{pending_count} rapportino/i in attesa</span>'
        f'</div>'
        f'<div style="font-size:11px; color:#92400e;">Verranno sincronizzati automaticamente quando online</div>'
        f'</div>',
        unsafe_allow_html=True
    )
    
    # Bottone per sincronizzazione manuale
    if conn_disponibile and gworksheet is not None:
        if st.sidebar.button("🔄 Sincronizza ora", use_container_width=True, key="sync_now_btn"):
...
if st.session_state.sync_in_progress:
    with st.spinner("🔄 Sincronizzazione in corso..."):
        ...
        for idx, pending in enumerate(st.session_state.pending_rapportini):
            try:
                # Aggiungi a rapportini se non già presente
                if pending not in st.session_state.rapportini:
                    st.session_state.rapportini.append(pending)
                if scrivi_su_google_sheets(gworksheet, st.session_state.rapportini):
                    synced_ok.append(pending)
                else:
                ...
                # Se fallisce, rimuoviamo da rapportini e teniamo in pending
                if pending in st.session_state.rapportini:
                    st.session_state.rapportini.remove(pending)
                synced_fail.append(pending)
        
        for item in synced_ok:
            if item in st.session_state.pending_rapportini:
                st.session_state.pending_rapportini.remove(item)
        
        if synced_ok:
            st.sidebar.success(f"✅ {len(synced_ok)} sincronizzato/i!")
        if synced_fail:
            st.sidebar.warning(f"⚠️ {len(synced_fail)} non sincronizzato/i")
        
        st.session_state.sync_in_progress = False

# Indicatore stato connessione nella sidebar
if conn_disponibile:
    st.sidebar.markdown(
        '<div style="display:flex; align-items:center; gap:4px; font-size:12px; color:#22c55e; margin-bottom:8px;">'
    ...
    st.sidebar.markdown(
        '<div style="display:flex; align-items:center; gap:4px; font-size:12px; color:#ef4444; margin-bottom:8px;">'
...
if menu == "Rapportini Aziendali":
    ...
    # Mostra avviso se ci sono pending
    if pending_count > 0:
        st.info(f"⏳ **{pending_count} rapportino/i** in attesa di sincronizzazione con Google Fogli. Verranno inviati automaticamente quando la connessione sarà disponibile.")
    ...
    if st.button("💾 Salva Rapportino", use_container_width=True):
        if cliente == "Seleziona cliente" or not cantiere:
            ...
            nuovo = {
                ...
                "ore": float(ore),
                "spese": float(spese),
                "nota_spesa": nota_spesa,
                "note": note
            }
            
            # Usa la funzione di salvataggio con gestione offline
            status, messaggio = salva_rapportino_con_offline(nuovo)
            
            if status == "success":
                st.success(messaggio)
            elif status == "pending":
                st.warning(messaggio)
                # Emetti JavaScript per salvare anche in IndexedDB (per persistenza oltre la sessione)
                st.markdown(f"""
                <script>
                if (window.RapportiniDB) {{
                    window.RapportiniDB.savePendingRapportino({json.dumps(nuovo)}).then(() => {{
                        console.log("[Rapportini Sync] Salvato in IndexedDB");
                        if (window.RapportiniSync) {{
                            window.RapportiniSync.updateBadge();
                        }}
                    }}).catch(err => {{
                        console.warn("[Rapportini Sync] Errore salvataggio IndexedDB:", err);
                    }});
                }}
                </script>
                """, unsafe_allow_html=True)
